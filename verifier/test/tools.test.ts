...
describe("agent-facing tools", () => {
  ...
  it("derives the same intent id for the same work, from any caller", () => {
    ...
    const a = createTools(env(null), audit);
    const b = createTools(env(null), []);
  ...
  it("verifies from a receipt and records the reason either way", async () => {
    ...
    assert.equal(audit.at(-1)!.outcome, "proven");
  });

  it("reports a mined-but-empty transaction as not proven", async () => {
    const t = createTools(env({ status: "0x1", blockNumber: "0x1", logs: [] }), audit);
    ...
    assert.equal(audit.at(-1)!.outcome, "not_proven");
  });
  ...
  it("refuses to settle an intent that is not open", async () => {
    ...
    const t = createTools(env(null), audit);
    ...
    assert.equal(audit.at(-1)!.outcome, "refused");
  });
